...
## Исправлено дополнительно (после первой сессии)

| # | Проблема | Статус |
|---|---|---|
| 1 | Страницы `/lots` и `/containers` — удалены list pages (page.tsx), оставлены detail pages (`[id]/page.tsx`) для deep linking | ✅ Исправлено |
| 2 | `operations/freeze/page.tsx` хардкодил bank_type='MCB' — добавлена автодетекция MCB/WCB через getBanks | ✅ Исправлено |
| 4 | schema.sql donors — поле `sex` уже было (не `gender`), проверено | ✅ Не требуется |
| 5 | `Checkbox` компонент — кастомная реализация уже существует в components/ui/checkbox.tsx | ✅ Не требуется |

### Дополнительные изменения:
- **`api.ts`** — `getBanks()` расширен фильтром `culture_id` для автодетекции MCB/WCB
- **`documents/page.tsx`** — ссылка "Перейти к лотам" → "Перейти к культурам" (/cultures)
- **`ТЗ_LabPro.md`** — полное обновление до версии 1.5: добавлены Донор, Донация, TissueType во все разделы (ERD, State Machine, бизнес-правила, справочники, API, UI, глоссарий)
- Удалены: `lots/page.tsx`, `lots/new/page.tsx`, `containers/page.tsx`
