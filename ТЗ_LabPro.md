...
5. **Нельзя создать культуру** — из донации со статусом отличным от `APPROVED`
6. **Объём/масса ткани** — зависит от формы ткани (`tissue_form`): LIQUID → объём (мл), SOLID → масса (г)
7. **Согласие донора** — рекомендуется, но не блокирует создание
8. **Нельзя удалить** — донацию можно только отклонить через результаты тестов
