...
- Контейнеры:
  ...
  - Выбирается из партий на складе (`batches`)
- Готовая среда:
...
- Тип клеток должен быть связан с типом ткани донации
...
- Списание контейнеров со склада: уменьшается `quantity` в `batches`, создаётся `inventory_movement` (CONSUME)
...
- Тип клеток определяется строго через связь `tissue_type → culture_type` (таблица `culture_type_tissue_types`). Свободный выбор запрещён.
...
- Контейнеры: опция "Списать со склада" — ищет подходящую партию расходников по имени типа контейнера, проверяет наличие.
