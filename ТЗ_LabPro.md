...
┌─────────────────────────────────────────────────────────────────┐
│                  QC_TEST_CONFIG (Справочник QC)                   │
├─────────────────────────────────────────────────────────────────┤
│ id: UUID                      PK                                │
│ code: TEXT                    Код (MYCOPLASMA, STERILITY, etc.) │
│ name: TEXT                    Название                          │
│ description: TEXT             Описание (nullable)               │
│ methodology: TEXT             Методика (nullable)               │
│ unit: TEXT                    Единица измерения (nullable)      │
│ ref_min: NUMERIC              Референс мин. (nullable)          │
│ ref_max: NUMERIC              Референс макс. (nullable)         │
│ result_type: TEXT             BINARY или NUMERIC                │
...
┌─────────────────────────────────────────────────────────────────┐
│            CULTURE_TYPE_QC_REQUIREMENTS (Привязка QC)             │
├─────────────────────────────────────────────────────────────────┤
│ id: UUID                      PK                                │
│ culture_type_id: UUID         FK -> CultureType ON DELETE CASCADE│
│ qc_test_config_id: UUID       FK -> QCTestConfig ON DELETE CASCADE│
...
7. **Списание контейнеров со склада (batches):**
   ...
   - Создаётся `inventory_movement` с типом `CONSUME`
8. **Валидация:**
   - Среды: если суммарного остатка **достаточно**, но откроется новый флакон — **предупреждение** (жёлтое)
   - Среды: если суммарного остатка **недостаточно** — **блокировка** (красное, кнопка «Сохранить» заблокирована)
...
- Фильтр по категории среды (опционально) — выпадающий список: Все / Среды / Сыворотки / Буферы / Ферменты / Реагенты / Добавки
- Партия среды ИЛИ Готовая среда (обязательно) — единый dropdown (📦 партии + 🧪 готовые среды), фильтрация по тегу FEED
- Объём на контейнер (мл, обязательно)
- Дополнительные компоненты (опционально, динамические строки):
  - Для каждого компонента: среда/реагент (dropdown) + объём (мл)
  - Используется для добавок: сыворотка, FGF, EGF и т.д.
...
- Создаются Operation_Container для каждого контейнера
- Создаётся Operation_Medium для основной среды
- Создаётся Inventory_Movement (списание, unit: 'мл')
- Списание дополнительных компонентов: для каждого — writeOff + operation_media + inventory_movement (unit: 'мл')
...
- Фильтр по категории среды (опционально) — выпадающий список: Все / Среды / Сыворотки / Буферы / Ферменты / Реагенты / Добавки
- Среды (каждый dropdown — 📦 партии + 🧪 готовые среды, фильтрация по usage_tag):
  - Диссоциация (партия или RM, тег DISSOCIATION)
  - Отмывка (партия или RM, тег WASH)
  - Питательная среда для посева (партия или RM, тег SEED)
- Дополнительные компоненты (опционально, динамические строки):
  - Для каждого: среда/реагент (dropdown) + объём (мл)
  - Применяется для сывороток, факторов роста, добавок
- Результат:
  - Тип контейнера (обязательно)
...
- Создаётся Operation_Medium для всех сред (диссоциация, отмывка, посев)
- Списание дополнительных компонентов: для каждого — writeOff + operation_media + inventory_movement (unit: 'мл')
- Создаётся Inventory_Movement для списания контейнеров со склада (unit: 'шт')
- Создаётся Operation_Metrics
- Пофлаконный учёт: writeOffBatchVolume() для всех сред (если привязаны к batch)
...
- Параметры:
  ...
  - Объём на криовиалу (мл, обязательно) — рабочий объём (может отличаться от номинального)
  - Количество криовиал (обязательно)
  - Партия криовиалов (выбор из склада, тег FREEZING)
- Фильтр по категории среды (опционально) — выпадающий список категорий
- Среды (📦 партии + 🧪 готовые среды, фильтрация по usage_tag):
  - Диссоциация (партия или RM, тег DISSOCIATION)
  - Отмывка (партия или RM, тег WASH)
  - Заморозка (партия или RM, тег FREEZING, обязательно)
- Дополнительные компоненты (опционально, динамические строки):
  - Для каждого: среда/реагент (dropdown) + объём (мл)
- Метрики:
  - Концентрация (клеток/мл, обязательно)
...
**Побочные эффекты:**
- **Полная заморозка** (все контейнеры лота выбраны):
  ...
  - Лот-источник обновляется: final_cells, viability, harvest_at, статус → CLOSED/BANKED
- **Частичная заморозка** (выбрана часть контейнеров, остальные IN_CULTURE):
  - Создаётся новый лот (parent_lot_id = исходный лот)
  - Замороженные контейнеры → IN_BANK, перемещаются в новый лот
  - Bank и CryoVial привязываются к новому лоту
  - Исходный лот остаётся ACTIVE с оставшимися IN_CULTURE контейнерами
- Криовиалы размещаются в указанной позиции
- Списание сред: writeOffBatchVolume для всех 3 сред (unit: 'мл')
- Списание криовиалов со склада (unit: 'шт')
- Списание дополнительных компонентов (unit: 'мл')
- Создаётся Operation (FREEZE)
- Создаётся Operation_Metrics (total_cells, viability, volume_ml)
- Создаётся QC_Task (автоматически из qc_test_configs)
...
- Фильтр по категории среды (опционально) — выпадающий список категорий
- Среды:
  - Разморозка (партия или RM, тег THAW, обязательно)
  - Объём среды (мл, обязательно)
- Контейнер для посева:
  - Партия контейнеров со склада (обязательно) — dropdown из getAllConsumableBatches()
- Дополнительные компоненты (опционально, динамические строки):
  - Для каждого: среда/реагент (dropdown) + объём (мл)
- Метрики:
...
- Lot.initial_cells = CryoVial.cells_count, Lot.viability = из cryo_vial
- Создаётся Container для каждой размороженной криовиалы
- CryoVial → ISSUED
- Списание среды разморозки: writeOff + operation_media + inventory_movement (unit: 'мл')
- Списание контейнера со склада (unit: 'шт')
- Списание дополнительных компонентов (unit: 'мл')
- Создаётся Operation (THAW)
- Создаётся Operation_Metrics (total_cells, viability)
...
**CRUD операции:**
- Создание номенклатуры (MEDIUM, SERUM, BUFFER, ENZYME, REAGENT, SUPPLEMENT, CONSUMABLE)
...
**Единица учёта (unit):**
- `'мл'` — для сред, реагентов, сывороток (пофлаконный учёт через writeOffBatchVolume)
- `'шт'` — для расходников: контейнеры (пассаж, разморозка), криовиалы (заморозка)

**Журнал расхода в карточке партии (inventory/[id]):**
- Таблица `inventory_movements` с колонками: Дата | Тип | Кол-во | Культура | Операция
- Для движений с `reference_type='OPERATION'` — JOIN через `reference_id` → `operations` → `lots` → `cultures`
- Колонка «Культура» — кликабельная ссылка на `/cultures/{id}` (код или название культуры)
- Колонка «Операция» — бейдж типа (Пассаж, Посев, Подкормка...) + номер лота + номер пассажа
- Позволяет оператору отслеживать: на какую культуру и в какой операции был израсходован расходник
...
3. **Добавки** — каждая добавка задаётся в процентах от общего объёма:
   - Фильтр по категории компонента (опционально): Все / Сыворотки / Буферы / Ферменты / Реагенты / Добавки (исключая MEDIUM и CONSUMABLE)
