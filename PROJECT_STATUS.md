...
**Версия:** 1.30.02
**Реализовано:** ~98% от ТЗ (25 фаз + 30+ итераций)
...
## Сессия 15.02.2026 (дом) — v1.28.00 → v1.30.02

### v1.30.02
- [x] Справочник: убрано поле «Фасовка» из номенклатуры — указывается при приёмке партии
- [x] Fix: единица измерения в остатках — вместо hardcoded «мл» используется реальная единица (мг, г, ЕД)

### v1.30.00–v1.30.01
- [x] STOCK калькулятор: расчёт по удельной активности (ЕД/мг) — масса × ЕД/мг → объём для целевой ЕД/мл
- [x] Справочник: поле «Удельная активность (ЕД/мг)» для ферментов
- [x] units.ts: calcTotalActivity(), calcVolumeForActivityConc(), calcActivityConc()
- [x] Приёмка: переработаны лейблы — «Кол-во упаковок», «Объём/Масса 1 упаковки»
- [x] Визуальная подсказка-пример при выборе номенклатуры (синий блок)
- [x] Итого-блок выделен зелёным цветом для наглядности

### v1.29.00–v1.29.02
- [x] Молярные единицы: моль, ммоль, мкмоль — в типах, справочнике, приёмке и калькуляторе
- [x] STOCK форма: подсказка-калькулятор молярных расчётов (MW → авто-пересчёт объёма/концентрации)
- [x] Молярные функции: toMoles(), calcVolumeForMolarConc(), calcMolarConc()
- [x] Справочник: поле «Фасовка» для media/reagents (content_per_package) — добавлено и убрано в v1.30.02
- [x] Приёмка: фасовка автозаполняется из справочника для всех категорий
- [x] Версия приложения отображается в header

### v1.28.00–v1.28.02 (Рефакторинг калькулятора — 4 режима)
- [x] Калькулятор: 4 режима — RECIPE, STOCK, DILUTION, ALIQUOT
- [x] RECIPE: unified dropdown (batch:UUID / rm:UUID), авто-списание растворителя + компонентов
...
- [x] Склад: табы [Все] [Контейнеры] [Поступления] [Стоки] [Готовые среды], бейдж «Аликвота»
- [x] Сворачиваемые группы аликвот на складе (N из M, суммарный объём)
- [x] Карточка среды: renderers для STOCK, ALIQUOT, RECIPE (solvent), legacy
- [x] CRITICAL fixes: writeOffBatchVolume '', race condition RM-XXXX, create record BEFORE write-off
- [x] E2E bugfixes: prefill URL, '__none__' FK, Rules of Hooks, unit sync
...
### Приоритет: Средний
1. **Шаблоны рецептов**: сохранение/загрузка рецептов для повторного использования
2. **Серийные разведения**: UI для автоматического создания серии разведений из стока

### Приоритет: Низкий (бэклог)
3. RBAC: permission matrix и role-based route protection
4. QC-справочник: проверить CRUD и привязку к типам культур
5. submitDilution: non-atomic operations (3 sequential DB writes) — needs server-side transaction
...
| API (api.ts) | ~5145 строк |
| Типы (index.ts) | ~873 строки |
| Утилиты (units.ts) | ~336 строк |
| version.ts | ~422 строки |
| SQL миграции | 19 файлов |
| Фаз разработки | 25 + 30+ итераций |
...
|------|-------|------------|
| `frontend/src/lib/api.ts` | ~5145 | Все API-функции (Supabase) |
| `frontend/src/types/index.ts` | ~873 | TypeScript-типы |
| `frontend/src/lib/units.ts` | ~336 | Конвертация единиц, молярные/активность расчёты |
| `frontend/src/app/page.tsx` | ~650 | Дашборд |
| `frontend/src/lib/version.ts` | ~422 | Версия + changelog |
